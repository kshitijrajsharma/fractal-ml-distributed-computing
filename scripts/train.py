...
def parse_args():
    ...
    parser.add_argument(
        "-e", "--executor-memory", type=int, default=8, help="Executor memory in GB"
    )
    parser.add_argument(
        "-d", "--driver-memory", type=int, default=2, help="Driver memory in GB"
    )
    parser.add_argument(
        "-c", "--executor-cores", type=int, default=2, help="Executor cores"
    )
    parser.add_argument(
        "-x", "--num-executors", type=int, default=2, help="Number of executors"
    )
    parser.add_argument(
        "-p",
        "--data",
        dest="data_path",
        default="/opt/spark/work-dir/data/FRACTAL",
        help="Data path",
    )
    parser.add_argument(
        "-f",
    ...
    parser.add_argument(
        ...
        "--output",
        dest="output_path",
        default=os.path.join(cwd, "results"),
...
def create_spark_session(args):
    ...
    builder = (
        ...
        .config(
            ...
            str((args.executor_cores * args.num_executors) * 4),
    ...
    logger.info(
        f"Spark session created successfully : executors={args.num_executors}, cores={args.executor_cores}, memory={args.executor_memory}g, fraction={args.sample_fraction}"
    )
